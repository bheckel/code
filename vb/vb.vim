...
" Modified: Thu 23 May 2002 10:22:25 (Bob Heckel)
...
syn keyword vbStatement And As AppActivate Base Beep ByRef ByVal Call Case ChDir ChDrive Const
...
syn keyword vbStatement DeleteSetting Dim Do Else ElseIf End Enum Erase Event Exit
syn keyword vbStatement Explicit False FileCopy For Each Function Get Global GoSub GoTo
syn keyword vbStatement If Implements In Kill Let LineInput Lock Loop LSet MkDir Name Next
syn keyword vbStatement Not Nothing OnError On Option Optional Or Preserve Private Property Public Put RaiseEvent
...
syn keyword vbStatement True Type Unlock Until Wend While Width With Write

syn keyword vbFunction Abs Array Asc AscB AscW Atn Avg Byte Boolean
syn keyword vbFunction CBool CByte CCur CDate CDbl Cdec Choose Chr ChrB ChrW
syn keyword vbFunction CInt CLng Command Cos Count CreateObject CSng CStr Currency
syn keyword vbFunction CurDir CVar CVDate CVErr Date DateAdd DateDiff DatePart
syn keyword vbFunction DateSerial DateValue Day Dictionary DDB Dir DoEvents Double Environ EOF
syn keyword vbFunction Error Exp FileAttr FileDateTime FileLen FileSystemObject Fix Format
...
syn keyword vbFunction InputBox InStr InstB Int Integer IPmt IsArray IsDate IsEmpty
syn keyword vbFunction IsError IsMissing IsNull IsNumeric IsObject LBound LCase
syn keyword vbFunction Left LeftB Len LenB LoadPicture Loc LOF Log Long LTrim Max
syn keyword vbFunction Mid MidB Min Minute MIRR Mod Month MsgBox Now NPer NPV Oct
syn keyword vbFunction Object Partition Pmt PPmt PV QBColor Rate RGB Right RightB Rnd
syn keyword vbFunction RTrim Single Second Seek Sgn Shell Sin SLN Space Split Spc Sqr StDev
...
syn keyword vbFunction Val Var Variant VarP VarType Weekday Year
...
syn keyword vbMethods CreateWorkspace Customize Debug Delete DeleteColumnLabels
syn keyword vbMethods DeleteColumns DeleteRowLabels DeleteRows DoVerb Drag Draw
syn keyword vbMethods Edit EditCopy EditPaste EndDoc EnsureVisible Exists
...
syn keyword vbMethods InsertObjDlg InsertRowLabels InsertRows Item Keys KillDoc
...
syn keyword vbMethods MoveFirst MoveLast MoveNext MovePrevious NavigateTo New NewPage
...
syn keyword vbMethods RegisterDatabase Reload Remove RemoveAll RemoveAddInFromToolbar
