...
'          centralized INI-format file, writing changes if prompted by user.
'          See modUtils.bas, modGlobals.bas, modErrorHandler.bas
'
'          To add new elements to form:
'            1 - Add gui widget, (Name) similar to the others, Tag equal to
'                corresponding element in config.dat
'
'            2 - If require initialization (e.g. comboboxes, listboxes, etc.),
'                add an array to SetWidgetDefaults() and loop over it.
'
...
' Modified: Tue, 12 Dec 2000 09:16:06 (Bob Heckel)
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Private Sub ProcessConfig
  ...
  If FileExists(CONFIGDAT) Then
    ...
    #End If
    ' Then load config.dat to populate defaults.  Ret array of each [segment].
...
ERRORTRAP:
  Dim moreinfo As String
  moreinfo = "Error while populating the widgets based on config.dat " & _ 
             "data.  Check that filesize is smaller than " & WARNSIZE
  Call Entropy(Err.Number, Err.Source, Err.Description, Err.HelpFile, _
...
Private Sub cmdHelpBtn_Click()
  ...
  ' TODO do this without using brute force, API call??
...
Sub ReadyAgain
  ...
  For i = 0 To tabTweak.Tabs -1
    ...
    lblStatus(i).ForeColor = &H000000&   ' Black.
  Next
End Sub
''''''''''''''''''
    ...
    Else
      For i = 0 To tabTweak.Tabs - 1
        lblStatus(i).Caption = "No changes have been made to " & CONFIGDAT & _
                               ".  Nothing to save."
        lblStatus(i).ForeColor = &H0000FF&   ' Dark red.
      Next
    End If
  End If
  Exit Sub
