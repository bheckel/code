...
options set=SAS_HADOOP_JAR_PATH="/sas/sashome/SAS_HADOOP_JAR_PATH";

proc sql;
  connect to hadoop (server='hwnode-02.hdp.ateb.com' schema=patient user=hive password=hive);
    create table t as select * from connection to hadoop (
      select distinct clientid from atebpatient
    );
...
create table junktbl (name string);
insert into junktbl select drivers.name from drivers where name like '%ettic%';
...
use patient;
describe atebpatient;
select * from patient.atebpatient where  clientid = 4 and atebpatientid = 22129149 ;
