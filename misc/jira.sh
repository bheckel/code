...
echo "-- WITHOUT GITHUB UI2: gco && gpul && gco ${JIRA} && git merge develop && gpul && gco && git merge --squash feature/ORION-${JIRA} && git add . && git commit -m 'ORION-${JIRA}: ' && git push && git branch -d feature/ORION-${JIRA} && git fetch -p && git push origin --delete feature/ORION-${JIRA}" >> ${JIRA}.sql
