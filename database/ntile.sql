...
--do in chunks
    ...
    SELECT account_id
      FROM ( select account_id, row_number() OVER (order by account_id) rn from rion_51396@sed )
     WHERE rn between 1 and 9;
      ...
      SELECT account_id, input_source
       ...
       WHERE account_id IN (
                            select distinct account_id
                              from ( select account_id, ntile(1) OVER (order by account_id) grp from rion_51396@sed ) 
                             where grp = 1
                            );
