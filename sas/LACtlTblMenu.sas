...
%MACRO LAAdminMenu;
	DATA _NULL_;
		...
		PUT 	'<input type="submit" name="Submit" value="  Read a LINKS PEC Table Specification "';
		...
		PUT 	'<input type="submit" name="Submit" value="Update a LINKS PEC Table Specification"';
		PUT 	' style="background-color: rgb(192,192,192); color: rgb(0,0,0);';
		PUT 	' font-size: 9pt; font-family: Arial ; font-weight: bolder;';
		PUT 	' letter-spacing: 0px; text-align: center; vertical-align: baseline; ';
		PUT 	' border: medium outset; padding-top: 2px; padding-bottom: 4px">';
...
%MACRO LACtlTblUpd;
	...
	DATA _NULL_; LENGTH line $1000; FILE _WEBOUT;
		...
		PUT 	'<TR align="LEFT" ><TD bgcolor="#003366">';
		line=	'<big><big><big><img src="http://'||"&_server"||'/links/images/gsklogo1.gif" width="141" height="62">'; put line;
		PUT 	'<font FACE=ARIAL color="#FFFFFF"><em> LINKS </em> PKS Extraction Control Table Module</FONT>';
		PUT 	'</big></big></big></TD></TR>';
		...
		PUT 	'</br></br><font FACE=ARIAL color="#003366"><em>This program will update the LINKS PEC table using the selected data file.</font></em>';
		PUT 	'</font></strong><p>'; 
...
%MACRO LACtlTblRd;
	...
	DATA _NULL_;
		...
		PUT 	'</br></br><em><font FACE=ARIAL color="#003366">This program will read the selected specification from the LINKS PEC table </br> and save it to a spreadsheet file.</em></font></br>';
	...
	%dropdown(datalist1,Meth_Spec_Nm);  /* Generate drop down list */
...
%MACRO help;
	DATA _NULL_; LENGTH LINE $1000;
		...
		PUT 	'<big><font FACE=ARIAL color="#003366">The LINKS PKS Extraction Control Table Module provides an interface for LINKS Administrators';
...
%MACRO Decide;
	...
	%IF %SUPERQ(menu)= 						%THEN %LAAdminMenu;
	   %ELSE %IF %SUPERQ(menu)=LACtlTblRd	%THEN %LACtlTblRd;
	   %ELSE %IF %SUPERQ(menu)=LACtlTblUpd	%THEN %LACtlTblUpd;
	   %ELSE %IF %SUPERQ(menu)=HELP			%THEN %HELP;
%MEND;
OPTIONS SOURCE MPRINT MLOGIC noSYMBOLGEN;
%GLOBAL INIT MENU;
OPTIONS	NOXWAIT;
