/* Functions to make fuzzy comparisons between strings
   ...
   Modified for efficiency by Bob Heckel <gnu.20.bheckel@recursor.net>
   gcc -Wall -c fstrcmp.c && gcc -Wall fstrcmp_test.c fstrcmp.o
*/
...
/* This corresponds to the diff -H flag.  With this heuristic, for strings
 * with a constant small density of changes, the algorithm is linear in the
 * strings size.  This is unlikely in typical uses of fstrcmp, and so is
 * usually compiled out.  Besides, there is no interface to set it true.  
 */
#ifdef MINUS_H_FLAG
...
diag - find diagonal path
  ...
  Set PART->LEFT_MINIMAL to nonzero if the minimal edit script
...
static int diag (int xoff, int xlim, int yoff, int ylim, int minimal, 
                 struct partition *part) {
  int *const fd = fdiag;  /* give the compiler a chance. */
  int *const bd = bdiag;  /* additional help for the compiler. */
  const char *const xv = string[0].data;  /* still more help */
...
#ifdef MINUS_H_FLAG
    /* Heuristic: check occasionally for a diagonal that has made lots of
     * progress compared with the edit distance.  If we have any such, find
     * the one that has made the most progress and return it as if it had
     * succeeded.  With this heuristic, for strings with a constant small
     * density of changes, the algorithm is linear in the strings size. 
     */
...
double fstrcmp (const char *string1, const char *string2, double minimum) {
  ...
  /* Set TOO_EXPENSIVE to be approximate square root of input size, bounded
   * below by 256.  
   */
  too_expensive = 1;
  ...
  /* Because fstrcmp is typically called multiple times, while scanning symbol
   * tables, etc, attempt to minimize the number of memory allocations
   * performed.  Thus, we use a static buffer for the diagonal vectors, and
   * never free them.  
   */
  ...
  max_edits = 1 + (string[0].data_length + string[1].data_length) * 
                                                             (1. - minimum);
